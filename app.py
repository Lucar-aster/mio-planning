...
def render_gantt_fragment(df_plot, color_map, oggi_dt, x_range, delta_giorni, shapes):
    ...
    for op in df_merged['operatore'].unique():
        ...
        fig.add_trace(go.Bar(
            ...
            pattern=dict(shape="/", fgcolor=df_op['tag'].astype(str).str.strip().str.lower().map(mappa_colori_tag).fillna("rgba(0,0,0,0)").tolist(), fgopacity=0.9, size=6,solidity=0.3, fillmode="overlay")), width=0.4,
