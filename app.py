...
# --- TAB 1: TIMELINE (SOLO DATE E SENZA ERRORI DI INDENTAZIONE) ---
with menu[0]:
    st.header("Timeline Lavori")
    
    with st.expander("➕ Registra rapidamente"):
        c_res = supabase.table("Commesse").select("id, nome_commessa").execute().data
        if c_res:
            col_c, col_t = st.columns(2)
            lista_c = {c['nome_commessa']: c['id'] for c in c_res}
            sel_c = col_c.selectbox("Progetto", options=list(lista_c.keys()), key="q_c")
            ...
            if t_res:
                ...
                sel_t = col_t.selectbox("Task", options=list(lista_t.keys()), key="q_t")
                
                with st.form("quick_log_form"):
                    op = st.text_input("Operatore", key="q_op")
                    d_i = st.date_input("Inizio", key="q_di")
                    d_f = st.date_input("Fine", key="q_df")
                    
                    if st.form_submit_button("Salva"):
                        supabase.table("Log_Tempi").insert({
                            "task_id": lista_t[sel_t], "operatore": op,
                            "inizio": d_i.isoformat(), "fine": d_f.isoformat()
                        }).execute()
    ...
    try:
        ...
        if res_logs:
            ...
            # Convertiamo e forziamo solo la data
            ...
            fig = px.timeline(df, x_start="inizio_data", x_end="fine_data", y="Task", 
                              color="operatore", text="operatore")
