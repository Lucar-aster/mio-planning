...
# --- 4. FUNZIONI UTILITY (TUE ORIGINALI) ---
...
def modal_manage_task_and_log(task_id, data_clic):
    ...
    c1, c2 = st.columns(2)
    with c1: op_sel = st.selectbox("Operatore", ops); d_ini = st.date_input("Inizio", value=data_clic)
    with c2: nota = st.text_input("Nota"); d_fin = st.date_input("Fine", value=data_clic)
...
# --- 8. TABS (POSIZIONE ORIGINALE SOPRA IL GRAFICO) ---
...
with tabs[0]: # GANTT
    ...
    def render_gantt_fragment(df_plot, oggi_dt, x_range):
        ...
        fig.update_layout(height=400+(len(df_tasks)*28), barmode='group', xaxis=dict(type="date", range=x_range, side="top"), yaxis=dict(autorange="reversed"), showlegend=False, margin=dict(l=10, r=10, t=40, b=0))
        ...
        while curr <= x_range[1] + timedelta(days=5):
            if curr.weekday() >= 5: fig.add_vrect(x0=curr, x1=curr+timedelta(days=1), fillcolor="#f0f0f0", opacity=0.3, layer="below", line_width=0)
...
with tabs[1]: # CALENDARIO
    if not df_p.empty:
        ...
        calendar(events=evs, options={"headerToolbar": {"left": "prev,next today", "center": "title", "right": "dayGridMonth,dayGridWeek"}}, key="cal_v7")

with tabs[2]: # SETUP COMMESSE
