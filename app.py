...
# --- NUOVA LOGICA: AREA DI CLIC GIORNALIERA DINAMICA ---
    ...
    while curr <= x_range[1] + timedelta(days=60):
        all_shapes.append(dict(type="line", x0=curr.replace(hour=8, minute=0), x1=curr.replace(hour=17, minute=0), y0=0, y1=1, yref="paper", line=dict(color="#e0e0e0", width=1), layer="below"))
        if curr.weekday() >= 5:
