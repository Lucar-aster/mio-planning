...
def get_it_date_label(d, delta_giorni):
    ...
    if delta_giorni <= 60: return f"{d.day} {mesi_it[d.month-1][:3]}"
...
def aggiorna_database_setup(table_name, df_new, raw_old):
    ...
    st.success(f"Database {table_name} aggiornato!"); get_cached_data.clear(); st.rerun()
...
def modal_manage_task_and_log(task_id, data_clic):
    ...
    with st.expander("Modifica Anagrafica", expanded=False):
        col_n = 'nome' if 'nome' in task_info else 'nome_task'
        new_tk_name = st.text_input("Nome Task", value=task_info.get(col_n, ''))
        ...
        if st.button("Salva Modifiche", use_container_width=True):
            supabase.table("Task").update({col_n: new_tk_name, "stato": new_tk_status}).eq("id", task_id).execute()
...
if not df_log.empty and not df_tk.empty and not df_cm.empty:
    col_tk_n = 'nome' if 'nome' in df_tk.columns else ('nome_task' if 'nome_task' in df_tk.columns else None)
    df = df_log.merge(df_tk[['id', col_tk_n, 'commessa_id', 'stato']], left_on='task_id', right_on='id', suffixes=('', '_tk'))
    df = df.merge(df_cm[['id', 'nome_commessa', 'stato']], left_on='commessa_id', right_on='id', suffixes=('', '_cm'))
    df = df.rename(columns={col_tk_n: 'Task', 'nome_commessa': 'Commessa', 'stato_cm': 'stato_commessa', 'stato': 'stato_task'})
...
def render_gantt_fragment(df_plot, oggi_dt, x_range):
    ...
    for _, rt in df_tasks.iterrows():
        ecm, etk = mappa_emoji.get(rt['stato_commessa'], "⚫"), mappa_emoji_task.get(rt.get('stato_task'), "⚫")
        cl = "<br>".join(textwrap.wrap(f"{ecm} {rt['Commessa']}", 15))
        yv = cl if st.session_state.vista_compressa else (cl, "<br>".join(textwrap.wrap(f"{etk} {rt['Task']}", 20)))
        fig.add_trace(go.Bar(base=[x_range[0]], x=[(x_range[1] - x_range[0]).total_seconds() * 1000], y=[yv], orientation='h', marker=dict(color="rgba(0,0,0,0)"), showlegend=False, hoverinfo='none', customdata=[["GHOST", rt['task_id']]], width=0.5))
    for op in df_merged['operatore'].unique():
        ...
        for _, r in df_op.iterrows():
            ...
            y_labels.append(cl if st.session_state.vista_compressa else (cl, "<br>".join(textwrap.wrap(f"{etk} {r['Task']}", 20))))
        fig.add_trace(go.Bar(base=df_op['Inizio'], x=df_op['Durata_ms'], y=y_labels, orientation='h', name=op, marker=dict(color=color_map.get(op, "#8dbad2"), cornerradius=12), width=0.4, customdata=[["LOG", r['id'], r['operatore'], r['Inizio'], r['Fine'], r['Commessa'], r['Task'], r['note_html'], r['task_id']] for _, r in df_op.iterrows()], hovertemplate="<b>%{customdata[5]} - %{customdata[6]}</b><br>%{customdata[2]}<extra></extra>"))
    shps = []
    curr = x_range[0] - timedelta(days=5)
    while curr <= x_range[1] + timedelta(days=5):
        if curr.weekday() >= 5: shps.append(dict(type="rect", x0=curr, x1=curr+timedelta(days=1), y0=0, y1=1, yref="paper", fillcolor="#f0f0f0", opacity=0.3, line_width=0, layer="below"))
        curr += timedelta(days=1)
    fig.update_layout(height=400 + (len(df_tasks) * 28), shapes=shps, barmode='group', xaxis=dict(type="date", range=x_range, side="top"), yaxis=dict(autorange="reversed"), showlegend=False, margin=dict(l=10, r=10, t=40, b=0))
...
with tabs[1]:
    if not df_p.empty:
        evs = [{"id": r['id'], "title": f"{r['operatore']} - {r['Task']}", "start": r['inizio'], "end": r['fine'], "allDay": True} for _, r in df_p.iterrows()]
        calendar(events=evs, options={"headerToolbar": {"left": "prev,next today", "center": "title", "right": "dayGridMonth,dayGridWeek"}}, key="cal_v8")
with tabs[2]:
    st.subheader("🏢 Gestione Commesse")
    ecm = st.data_editor(df_cm, column_config={"id": None, "stato": st.column_config.SelectboxColumn("Stato", options=STATI_COMMESSA)}, num_rows="dynamic", hide_index=True, key="ed_cm_v8")
    if st.button("Salva Commesse", key="btn_cm_v8"): aggiorna_database_setup("Commesse", ecm, raw_cm)
with tabs[3]:
    ...
    if not df_tk.empty and not df_cm.empty:
        ...
        etk = st.data_editor(dftk_s, column_config={"id": None, "commessa_id": None, "commessa_nome": st.column_config.SelectboxColumn("Commessa", options=list(n2id.keys())), "stato": st.column_config.SelectboxColumn("Stato", options=STATI_TASK)}, num_rows="dynamic", hide_index=True, key="ed_tk_v8")
        if st.button("Salva Task", key="btn_tk_v8"):
...
with tabs[4]:
    if not df_p.empty:
        ...
        c2.plotly_chart(go.Figure(data=[go.Pie(labels=df_p['stato_commessa'].value_counts().index, values=df_p['stato_commessa'].value_counts().values)]), use_container_width=True)
