    ...
    with st.expander(f"⏱️ Nuovo Log - {data_clic.strftime('%d/%m/%Y')}", expanded=True):
        ...
        if c1.button("Registra Log", type="primary", width='stretch', key="regista_l"):
            ...
            if not op_sel_l or len(date_range_l) < 2: st.warning("Seleziona operatore e range date.")
                ...
                nuovi_log_l = [{"task_id": task_id, "operatore": op, "inizio": str(data_inizio_l), "fine": str(data_fine_l), "ora_i": str(ora_i_l), "ora_f": str(ora_f_l), "note": nota_l} for op in op_sel_l]
...
if l and tk and cm:
    ...
    df['Durata_ms'] = (df['Visual_Fine'] - df['Visual_Inizio']).dt.total_seconds() * 1000
