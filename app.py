...
def modal_gestione_clic(task_id, data_clic):
    ...
    with st.expander("📑 Nuovo Task con Log", expanded=False):
        ...
        if auto_orait:
            ora_i_t = datetime.now(tz).strftime('%H:%M')
        ...
        if c1.button("Registra Task", type="primary", width='stretch'):
            if not op_sel_t or len(date_range_t) < 2: st.warning("Seleziona operatore e range date valido.")
                ...
                nuovi_log_t = [{"task_id": final_task_id, "operatore": op, "inizio": str(data_inizio_t), "fine": str(data_fine_t), "ora_i": str(ora_i_t), "ora_f": str(ora_f_t), "note": nota_t} for op in op_sel_t]
    ...
    with st.expander(f"⏱️ Nuovo Log - {data_clic.strftime('%d/%m/%Y')}", expanded=True):
        ...
        if auto_orail:
            ora_i_l = datetime.now(tz).strftime('%H:%M')
            st.info(f"Verrà registrato l'orario d'inizio: {ora_i_t.strftime('%H:%M')}")
        else:
        ...
        if c1.button("Registra Log", type="primary", width='stretch', key="regista_l"):
            ...
            if not op_sel_l or len(date_range_l) < 2: st.warning("Seleziona operatore e range date.")
                ...
                nuovi_log_l = [{"task_id": task_id, "operatore": op, "inizio": str(data_inizio_l), "fine": str(data_fine_l), "ora_i": str(ora_i_l), "ora_f": ora_f_l.strftime('%H:%M:%S') if ora_f_l else None, "note": nota_l} for op in op_sel_l]
